      ...
      <div style={{ minHeight: '100vh', backgroundColor: '#F5F5F5', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '48px 40px', width: '100%', maxWidth: '420px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
          ...
          <input
            ...
            style={{ width: '100%', padding: '14px 16px', backgroundColor: '#1A1A1A', color: '#ffffff', border: 'none', borderRadius: '10px', fontFamily: "'Pretendard', sans-serif", fontSize: '16px', textAlign: 'center', letterSpacing: '2px', outline: 'none', boxSizing: 'border-box' }}
          />
          {error && (
          ...
          <button
            onClick={handleLogin}
            disabled={loading}
            style={{ width: '100%', marginTop: '20px', padding: '14px', backgroundColor: '#FF1659', color: '#ffffff', border: 'none', borderRadius: '10px', fontFamily: "'Pretendard', sans-serif", fontSize: '15px', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}
          >
            {loading ? '확인 중...' : '접속'}
          </button>
          <div style={{ borderTop: '1px solid #F0F0F0', marginTop: '28px', paddingTop: '20px', textAlign: 'center', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <p style={{ fontFamily: "'Pretendard', sans-serif", fontSize: '13px', color: '#aaa' }}>접속 코드는 슈퍼어드민에게 문의</p>
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#F5F5F5' }}>
          ...
          <div style={{ display: 'flex', gap: '4px' }}>
            {visibleTabs.map((tab) => (
              <button
                ...
                style={{ fontFamily: "'Pretendard', sans-serif", fontSize: '13px', fontWeight: activeTab === tab.key ? 700 : 500, color: activeTab === tab.key ? '#ffffff' : '#555', backgroundColor: activeTab === tab.key ? '#333333' : 'transparent', border: '1px solid', borderColor: activeTab === tab.key ? '#333333' : '#E0E0E0', borderRadius: '8px 8px 0 0', padding: '8px 16px', cursor: 'pointer', transition: 'all 0.15s', borderBottom: activeTab === tab.key ? '1px solid #ffffff' : '1px solid #E0E0E0', marginBottom: '-1px' }}
              >
                {tab.label}
              </button>
            ))}
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 40px' }}>
        {activeTab === 'users' && <AdminUsers currentAdmin={admin} />}
